...
// It's bidirectional machine 
public class Cobas6000 implements SerialPortEventListener {
	private static final Logger logger = Logger.getLogger(Cobas6000.class);
		...
		try {
			...
			//if(strReceived != null) {
                        if(event.isRXCHAR() && event.getEventValue() > 0) { // if data is available
                            ...
                            String strReceived = serialPort.readString(bytesCount);
	...
	public synchronized void saveData(String str) {
		...
		try {
			...
			logger.error("saveData Exception", ex);
			System.out.println("Data saving Exception:" + ex.getMessage());
		}
	}
	...
	public void SendTest(String qPatientID, SerialPort serialPort) {
		try {
			...
			if (patienList.size() > 0 && patienList.get(0).getBarcode() != null) {
